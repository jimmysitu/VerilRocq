...
Section HMap.
  ...
  (* Inductive HSub: hmap -> hmap -> Prop := *)
  (* | HSubEmpty: forall h, HSub HMapEmpty h *)
  (* | HSubBits: forall b, HSub (HMapBits b) (HMapBits b) *)
  (* | HSubArr: forall hs1 hs2, *)
  (*     Forall (fun sh1 => In sh1 hs2) hs1 -> *)
  (*     HSub (HMapArr hs1) (HMapArr hs2) *)
  (* | HSubStr: forall hs1 hs2, *)
  (*     (* Forall (fun sh1 => haccessV hs2 (fst sh1) = Some (snd sh1)) hs1 -> *) *)
  (*     Forall (fun sh1 => In sh1 hs2) hs1 -> *)
  (*     HSub (HMapStr hs1) (HMapStr hs2). *)
  ...
  (** Well-formedness: no duplicate keys at any level *)
  Inductive HMapWf : hmap -> Prop :=
    | Wf_Empty : HMapWf HMapEmpty
    | Wf_Bits : forall b, HMapWf (HMapBits b)
    | Wf_Arr : forall hs,
        NoDup (List.map fst hs) ->
        Forall (fun p => HMapWf (snd p)) hs ->
        HMapWf (HMapArr hs)
    | Wf_Str : forall hs,
        NoDup (List.map fst hs) ->
        Forall (fun p => HMapWf (snd p)) hs ->
  ...
  Section Facts.
    ...
    Lemma hbinUArr2_refl_nil: forall hs, hbinUArr2 hs hs = nil.
    Proof using .
      intros. apply filter_negb_existsb_nil.
      intros. apply existsb_In with (a:=x); auto.
      apply Z.eqb_refl.
    Qed.

    Lemma hbinUStr2_refl_nil: forall hs, hbinUStr2 hs hs = nil.
    Proof using .
      intros. apply filter_negb_existsb_nil.
      intros. apply existsb_In with (a:=x); auto.
      apply vid_eqb_refl.
